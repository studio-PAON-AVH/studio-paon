...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="int|tit" mode="asTitle">
        <xsl:apply-templates select="node()"/>
    </xsl:template>
    ...
    <!--
		...
		-->
    <xsl:template match="LIVRE">
        <xsl:variable name="titre" select="string-join(INFO/TITRE/text(), ', ')"/>
        <xsl:variable name="auteur" select="string-join(INFO/AUTEUR/text(), ', ')"/>
        <xsl:variable name="editeur" select="string-join(INFO/EDITEUR/text(), ', ')"/>
        <xsl:variable name="ISBN" select="concat('978',substring-after(string-join(INFO/ISBN/text(),''), '978'))"/>
        <dtbook version="2005-3" xmlns="http://www.daisy.org/z3986/2005/dtbook/" xml:lang="fr-FR">
            <head>
