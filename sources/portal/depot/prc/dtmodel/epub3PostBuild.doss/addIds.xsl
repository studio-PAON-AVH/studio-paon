...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="xhtml:section/@id"><xsl:attribute name="id"><xsl:value-of select="$dPfx"/><xsl:value-of select="."/></xsl:attribute></xsl:template>

	<xsl:template match="xhtml:p|xhtml:h1|xhtml:h2|xhtml:h3|xhtml:h4|xhtml:h5|xhtml:h6">
		<xsl:copy>
			<xsl:variable name="divBlock" select="ancestor::xhtml:section[@id][1]"/>
